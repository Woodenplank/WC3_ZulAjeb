...
do
    ...
    do
        ...
        ------------------------------ Modification to remove the OnInit library requirement--------------------------------------
        --                             Using Bribe's TotalInitialization instead                                                -- 
        local function dummysetup()
            local timer = CreateTimer()
            ...
            TimerStart(timer, 0, false, function()
                for i = 0, SWEET_SPOT do
                    ...
                    GroupAddUnit(group, unit)
                    UnitRemoveAbility(unit, FourCC('Amrf')) -- Amrf == "Crow Form"
                end
                PauseTimer(timer)
                DestroyTimer(timer)
            end)
        end
        local function CreateDummySetup()
            local trg = CreateTrigger()
            TriggerAddAction(trg, dummysetup)
        end
        OnInit.global(CreateDummySetup)
        --                Original code below
        -- onInit(function()
        --     local timer = CreateTimer()
